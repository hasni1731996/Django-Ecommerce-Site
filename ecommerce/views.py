...
def login_admin(request):
	...
	if request.method == 'POST':
		...
		if user is not None:
			login(request, user)
			return HttpResponseRedirect(Helpers.get_path('admin_login/orders'))
		return render(request, Helpers.admin_url('login.html'),{'data': 'password or username is wrong'})

@csrf_exempt
def adminsorder(request):
	err_succ = {'message': 'An unknown error occured',}
	if request.method ==  'GET':
		print('admin func')
		products=Product_orders.objects.all()
		return render(request, Helpers.admin_url('orders.html'),{'context':products,'currency': EcommerceConfig.currency})
	...
	if request.method == 'POST':
		...
		print('data...',data)
		order_obj=Product_orders.objects.get(order_id=data['order_id'])
		if order_obj.order_status == True:
			print('status...true',order_obj.order_status)
			order_obj.__dict__.update(order_status=False) 
			order_obj.save()
			err_succ['message'] = 'Order Status Updated Successfully'
			return JsonResponse(err_succ)
		else:
			print('status..false..',order_obj.order_status)
			order_obj.__dict__.update(order_status=True) 
			order_obj.save()
			err_succ['message'] = 'Order Status Updated Successfully'
			return JsonResponse(err_succ)
...
def admin_product(request,product_id):
	prod_obj=Product_orders.objects.filter(order_id=product_id).prefetch_related('product')
	order_obj=Product_orders.objects.get(order_id=product_id)
	products=Product.objects.filter(products_order__in=prod_obj)
	products_qty = zip(products, order_obj.prods_qty)
	return render(request, Helpers.admin_url('order_detail.html'),{'prods': products_qty, 'amount':order_obj.total_amount, 'order_id':order_obj.order_id, 'currency': EcommerceConfig.currency})
...
def orderuser(request):
	...
	if request.method ==  'GET':
		print('admin func')
		products=Product_orders.objects.filter(user=request.user)
		return render(request, Helpers.admin_url('orders.html'),{'context':products,'currency': EcommerceConfig.currency})
...
def single_product(request, product_id):
	...
	images = []
	in_cart = True if (User_cart.objects.filter(product=product_id,user=request.user)) else False
	if product_images:
	...
	if request.method == 'POST':
		...
		if all_posts:
			...
			for post in all_posts:
				print(User_cart.objects.filter(product=post.id))
				in_cart = True if (User_cart.objects.filter(product=post.id,user=request.user)) else False
...
def cart(request):
	...
	if request.method == 'POST':
		...
		if action == 'edit':
			...
			User_cart.objects.filter(product=item_id,user=request.user).update(product_quantity=quantity,total_price=total)
		
		# Add item 
		elif action == 'add':
			
			total=product_obj.price * quantity
			User_cart.objects.create(product=product_obj,user=request.user,product_quantity=quantity,total_price=total)
		...
		# Delete an item
			...
			try:
				...
				print(User_cart.objects.filter(product=item_id,user=request.user))
				User_cart.objects.get(product=item_id,user=request.user).delete()
			except KeyError:
				pass
	...
	# No post data availabe, let's just show the page.
		...
		user_cart = User_cart.objects.filter(user=request.user)
		cart_total = 0
		for item in user_cart:
			cart_total = item.total_price + cart_total
	
		return render(request, Helpers.get_url('cart.html'), {'cart_db_items': user_cart, 'cart_total': float("%0.2f" % (cart_total)), 'currency': EcommerceConfig.currency})
...
def checkout(request):
	if request.user.is_authenticated:
		...
		#item_ids = User_cart.objects.filter(user=request.user).values('product')
		user_cart = User_cart.objects.filter(user=request.user)
		#products = Product.objects.filter(pk__in=item_ids)
		...
		for item in user_cart:
			#cart_item = item.id
			#cart_total = (item.price * cart_item.quantity) + cart_total
			cart_total = item.total_price + cart_total
			
			# return HttpResponse(result)
		#creat_order(request.user,user_cart,float("%0.2f" % (cart_total)))
		return render(request, Helpers.get_url('checkout.html'), {'cart_db_items': user_cart, 'cart_total': float("%0.2f" % (cart_total)), 'currency': EcommerceConfig.currency})
	else:
...
def creat_order(request):
	...
	if request.method == 'POST':
		try:
			body_data = request.body.decode('utf-8')
			data = json.loads(body_data)
			user = Member.objects.filter(user=request.user).update(
			billing_first_name = data['billing_first_name'],
			billing_last_name = data['billing_last_name'],
			street_address = data['billing_stret_1'],
			...
			additional_info = data['billing_notes']
			)
			
			prod_obj = Product_orders.objects.create(user=request.user,total_amount=data['cart_total'],prods_qty=data['prods_qty'])

			prod = Product.objects.filter(id__in=data['products'])
		
			for i in prod:
				print('id....',i.id)
				prod_obj.product.add(i.id)

			err_succ['status'] = 1
			err_succ['message'] = 'Order Created'
			User_cart.objects.filter(user=request.user).delete()
			return JsonResponse(err_succ)
		except:
			err_succ['status'] = 0
			err_succ['message'] = 'User Not Get'
			return JsonResponse(err_succ)
...
def user_payment(request):
	...
	headers = {
		...
		'Authorization': 'YzU5YTIzZTNkZTFkMjA4OGY4MTllOTg5MmM2YzAyNjRkNmZjNGMyOGM1NGE4ZGYxOWJlNWU5YTFkMjhhODgxMg',
		'nonce': '2483502906806410000',
		'timestamp': '1546945436097'
	}

	r = requests.post(url, data=json.dumps(payload), headers=headers)
	print('resason', r.content)
	print('status', r.status_code)
	if r.status_code == 201:
		err_succ['status'] = 1
		err_succ['message'] = 'Payment Successfull'
		return JsonResponse(err_succ)
	else:
		err_succ['message'] = 'Error Creating Payment'
		return JsonResponse(err_succ)
####### Ends ########
...
def user_product_create(request):
	err_succ = {'status': 0, 'message': 'An unknown error occured','id':0 ,'images': []}
	...
	if request.method == 'POST':
		if form.is_valid():	
			...
			product = Product.objects.create(
				...
				category = Product_Category.objects.get(name = form.cleaned_data['category']),
			...
			if request.FILES.getlist('images'):	
				...
				err_succ['status'] = 1
				err_succ['id'] = product.id
				err_succ['message'] = 'Product successfully created'
	...
	if request.method == 'POST':
		if form.is_valid():		
			...
			if product.author != request.user.id:
				...
				product.category = Product_Category.objects.get(name = form.cleaned_data['category'])
...
def virtual_glass(request):
	return render(request, Helpers.glasses_url('demoBasic.html'))
